...
const getDetailsPagePaths = async (page = 0): Promise<string[]> => {
  ...
  const anc = doc.querySelectorAll('.shopsearchlist01 a') as NodeListOf<HTMLAnchorElement>;
...
const getDetailsPageData = async (path: string):Promise<ShopData> => {
  ...
  const tds = doc.querySelectorAll('.ShopSearchDetail_table tr td:nth-child(2)');
  const [
    type1,
    type2,
    businessType,
...
const main = async () => {
  ...
  await fs.writeFile('./data/shops.json', JSON.stringify(shopDataList, null, 2));
}
